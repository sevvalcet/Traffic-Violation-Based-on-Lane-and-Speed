...
from utils import letterbox, scale_coords, postprocess,BBoxTransform, ClipBoxes,cutout, restricted_float, \
    boolean_string, Params

from plot import STANDARD_COLORS, standard_to_bgr, get_index_label, plot_one_box
import os
...
parser.add_argument('--cpu', type=boolean_string, default=True)
...
img_path = glob(f'{source}/*.jpeg') + glob(f'{source}/*.png')
...
# cv2.imwrite('ori.jpeg', ori_imgs[0])
# cv2.imwrite('normalized.jpeg', normalized_imgs[0]*255)
...
for ori_img in ori_imgs:
    ...
    # cv2.imwrite('input.jpeg', input_img * 255)
...
with torch.no_grad():
    ...
    for i in range(seg.size(0)):
        ...
        for seg_class_index, seg_mask in enumerate(seg_mask_list):
            ...
            seg_mask_ = cv2.resize(seg_mask_, dsize=shapes[i][0][::-1], interpolation=cv2.INTER_NEAREST) # bunun içerisindeki elemanlar road (seg_class_index==0) ve lane(seg_class_index==1) e ait
            ...
            # cv2.imwrite('seg_only_{}.jpeg'.format(i), color_seg)
            ...
            seg_filename = f'{output}/{i}_{params.seg_list[seg_class_index]}_seg.jpeg' if seg_mode == MULTILABEL_MODE else \
                           f'{output}/{i}_seg.jpeg'
            ###########################################################################################################
           

                                ###########################################################################################################calibration
            # image=cv2.imread("/home/irem/HybridNets/demo/image/4.jpeg")
            # window_name = 'image'
            ...
            if np.where(seg_mask_==1):
                ...
                lane=mp.plot(x, y, color="black", linewidth=3)
                x1 = [388, 581]
                y1 = [-201, -398]
                lane1=mp.plot(x1, y1, color="red", linewidth=3)
                ...
                # mp.imshow(seg_img)
                # mp.show()
                # linees=cv2.line(seg_img, (328,132), (width,height), (255,0,0), 28)

                # cv2.line(seg_img, (0,0), (width, height), (0,0,255), 12)
    ...
    #             lanes = np.zeros((h0, w0, 3), dtype=np.uint8)
    #             lanes[seg_mask_==2] = [0, 255, 0]

    #             lanes_on_original = ori_img
    #             lanes_on_original=cv2.resize(lanes_on_original,(w0,h0))
    ...
    for i in range(len(ori_imgs)):
        out[i]['rois'] = scale_coords(ori_imgs[i][:2], out[i]['rois'], shapes[i][0], shapes[i][1]) # vehicle bbx
        ...
        if show_det:
            cv2.imwrite(f'{output}/{i}_det.jpeg',  cv2.cvtColor(det_only_imgs[i], cv2.COLOR_RGB2BGR))
        ...
        if imwrite:
            cv2.imwrite(f'{output}/{i}.jpeg', cv2.cvtColor(ori_imgs[i], cv2.COLOR_RGB2BGR))
