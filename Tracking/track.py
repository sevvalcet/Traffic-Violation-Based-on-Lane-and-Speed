...
def run(
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        for i, det in enumerate(p):  # detections per image
            ...
            #if fnc.intersect(tracker_list[1],tracker_list[2],start_point,end_point):
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        konum1 = {0:float(bbox[2]),1:float(bbox[1])}
                        konum2 = {0:float(bbox[2]),1:float(bbox[3])}


                        konum3 = {0:float(bbox[2]),1:float(bbox[1])}
                        konum4 = {0:float(bbox[2]),1:(float(bbox[1])+10)}



                        if(int(cls) >=7 ):
                            if fnc.intersect(konum3 ,konum4 ,start_point ,end_point):
                                
                                print(frame_idx)
                                cv2.line(im0, start_point, end_point, (0,0,0), thickness)
                        
                                #frame[id][counter]=frame_idx
                                frame = int(frame_idx)
                                print(frame)
                                


                            if fnc.intersect(konum3 ,konum4 ,start_point1 ,end_point1):

                                cv2.line(im0, start_point1, end_point1, (0,0,0), thickness)
                                frame1 = int(frame_idx)
                                print(frame1)
                                                                
                                time = (frame-frame1)/23
                                speed = ((konum1[1]-konum2[1])*20)/time
                                print("----------------------------------------------------------------------")
                                print(speed) 


                            if frame != 0 and frame1 != 0:
                                time = (frame-frame1)/23
                                speed = ((konum1[1]-konum2[1])*20)/time
                                print("---------------------------------------------------------------------")
                                print(speed) 
                        ...
                        ########################################################################################
                        #python3 track.py --yolo-weights best1.pt --tracking-method bytetrack  --source test1.mp4 --show-vid --save-vid
                        ...
                        # if(int(cls) >=7 ):
                        #     while fnc.intersect(konum1 ,konum2 ,start_point ,end_point):
                        ...
                        # pixelpermeter=20
                        # frame_number = ilk_frame - son_frame
                        # time=frame_number/23
                        # speed=(bbox_h*pixelpermeter)/time
